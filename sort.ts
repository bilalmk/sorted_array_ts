...
function welcome()
{
    ...
    console.log(`
        ${chalk.blue.bold.underline("Program Rules")}
...
function array_sort(sortType:string, ...arr:number[]):number[]
{
    let temp:number;
...
async function main()
{
    ...
    let inputSortList = input.sortList.split(",").map((x:string)=>parseInt(x));
